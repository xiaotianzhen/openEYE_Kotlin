        ...
        var observer=object :Observer<Any>{
            ...
            override fun onNext(model: Any?) {
                ...
                for(i in 1 until dataArray.length()){
                    ...
                    when(data.getString("type")){
                        "horizontalScrollCard"->{
                            Log.i("yicooll item",dataArray[i].toString())
                            //items.add(ModelTopBanner(data.getString()))
                        }
                        "specialSquareCardCollection"->{
                            var model: ModelCategoryCart=gson.fromJson(dataArray[i].toString(),ModelCategoryCart::class.java)
                            items.add(model as Object)
                        }
                        "columnCardList"->{
                           var model : ModelSubjectCard=gson.fromJson(dataArray[i].toString(),ModelSubjectCard::class.java)
                            items.add(model as Object)
                        }
                        "textCard"->{
                            var model : ModelTitleView=gson.fromJson(dataArray[i].toString(),ModelTitleView::class.java)
                            items.add(model as Object)
                        }
                        "banner"->{
                             var model: ModelBanner=gson.fromJson(dataArray[i].toString(),ModelBanner::class.java)
                            bannerList.add(model as Object)
                        }
                        "videoSmallCard"->{
                            var model : ModelVideoSmallCard=gson.fromJson(dataArray[i].toString(),ModelVideoSmallCard::class.java)
                            items.add(model as Object)
                        }
                        "briefCard"->{
                            var model : ModelBriefcard=gson.fromJson(dataArray[i].toString(),ModelBriefcard::class.java)
                            items.add(model as Object)
                        }
                    }
                ...
                if(bannerList.size>0){
                    items.add(bannerList as Object)
                }
